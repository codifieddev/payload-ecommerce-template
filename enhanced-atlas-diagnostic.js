...
async function comprehensiveDiagnostic() {
  ...
  console.log("- Private Key Format:", PRIVATE_KEY.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/) ? "✅ Valid UUID" : "❌ Invalid UUID");
  ...
  try {
    ...
    if (response.ok) {
      ...
      const clustersRes = await fetch(`https://cloud.mongodb.com/api/atlas/v1.0/groups/${PROJECT_ID}/clusters`, {
        method: "GET",
        headers: {
          "Authorization": `Basic ${auth}`,
          "Accept": "application/json",
